...
int move(char *old_path, char *new_path){
	...
	int parent_old_inumber = lookup(parent_name_old);
	int parent_new_inumber = lookup(parent_name_new);
	...
	if(dir_reset_entry(parent_old_inumber,child_inumber_old) == FAIL){
		printf("Could not reset entry\n");
		return FAIL;
	}
	if(dir_add_entry(parent_new_inumber,child_inumber_old,child_name_new) ==FAIL){
